...
# __version__ is hardcoded (and checked against pyproject by the test suite)
init = Path("hotdata_materialized/__init__.py")
text = init.read_text()
new, n = re.subn(r'(?m)^__version__ = "[^"]+"', f'__version__ = "{ver}"', text, count=1)
if n != 1:
    raise SystemExit("could not update __version__ in hotdata_materialized/__init__.py")
init.write_text(new)
PY
}
...
cmd_prepare() {
  ...
  git add pyproject.toml CHANGELOG.md hotdata_materialized/__init__.py
